...
| **Supported kernels** | GEMM, Flash Attention (FA2/FA3), softmax, layer norm, RMS norm, fused attention, MoE gating, element-wise fused ops, scan/reduction, dropout+bias fusions | GEMM (incl. batch, int4 dequant, split-K), Flash Attention, softmax, layer norm, MLA decoding, FlashMLA, element-wise ops, reduction; **Ascend:** GEMM, batch GEMM, conv, Flash Attention, softmax, normalization, activation, GEMV, grouped GEMM, sparse FA, pos. embedding, cross-entropy, linear attention/RNN, top-k | GEMM (matmul), Flash Attention (TPU/GPU), layer norm, softmax, element-wise ops, scan/reduction, sparse kernels, custom attention masks (FlexAttention style) | GEMM, vector add; attention/softmax via tile abstractions — limited published examples (early Blackwell-only, Dec 2025) | GEMM (15 LOC), softmax (2.28x torch.compile), RMSNorm (bwd), JSD, int4-GEMM, Mamba-2 chunk-scan, Flash Attention, fused element-wise ops | FMHA (Flash Attention style, B200/H100); pipelined attention with warp specialization; MMA (WGMMA); TMA load/store; experimental, not for general use | GEMM, matmul, stencil, BabelStream (memory ops), element-wise; softmax/attention via custom fused kernels (no official examples yet); primary focus: systems-level portability |
