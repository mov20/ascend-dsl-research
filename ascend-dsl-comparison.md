...
| **Target HW** | NVIDIA GPU, AMD GPU, Intel GPU (Xe); **Ascend** (via Triton-Ascend); **Meta MTIA** (via triton-mtia) | NVIDIA GPU, AMD GPU, **Huawei Ascend A2/A3** <sup>[[2]](#ref-2)</sup> | **Google TPU** (v4/v5/v6, native via Mosaic), NVIDIA/AMD GPU (via Triton) | NVIDIA GPU (Blackwell/Ampere+) <sup>[[4]](#ref-4)</sup> | NVIDIA, AMD, Intel GPU (via Triton); **Meta MTIA** (via triton-mtia) <sup>[[5]](#ref-5)</sup> | NVIDIA GPU (H100, B200) <sup>[[6]](#ref-6)</sup> | NVIDIA, AMD GPU, CPU <sup>[[7]](#ref-7)</sup> |
...
| <a name="ref-17"></a>[17] | [Triton 06-fused-attention.py](https://github.com/triton-lang/triton/blob/main/python/tutorials/06-fused-attention.py) · [perf benchmark](https://triton-lang.org/main/getting-started/tutorials/06-fused-attention.html) | Fwd kernel: inner helper 65L + main 71L = ~136L; benchmark vs cuDNN included in tutorial |
| <a name="ref-18"></a>[18] | [TileLang example_mha_fwd_bshd.py](https://github.com/tile-ai/tilelang/blob/main/examples/flash_attention/example_mha_fwd_bshd.py) · [FlashMLA benchmark](https://github.com/tile-ai/tilelang/blob/main/examples/deepseek_mla/README.md) | GPU fwd kernel: ~74L (lines 23–96); FlashMLA perf: matches/exceeds FlashMLA on H100 batch 64–128 |
| <a name="ref-19"></a>[19] | [TileLang-Ascend flash_attn_bhsd.py](https://github.com/tile-ai/tilelang-ascend/blob/ascendc_pto/examples/flash_attention/flash_attn_bhsd.py) | Ascend fwd kernel: ~208L (lines 8–215); no published perf numbers vs AscendC (Mar 2026) |
| <a name="ref-20"></a>[20] | [JAX Pallas tpu/flash_attention.py](https://github.com/jax-ml/jax/blob/main/jax/experimental/pallas/ops/tpu/flash_attention.py) | TPU fwd: 1718L total (production-grade, highly parameterized); ~3x speedup vs naive MHSA on TPUv3-8 |
| <a name="ref-21"></a>[21] | [Helion examples/attention.py](https://github.com/pytorch/helion/blob/main/examples/attention.py) · [Paged Attention blog](https://pytorch.org/blog/portable-paged-attention-in-helion/) | Kernel: ~81L (lines 35–115); Paged Attention: 133L vs 295L in Triton (2.2x fewer LOC) |
| <a name="ref-22"></a>[22] | [Gluon 08-warp-specialization.py](https://github.com/triton-lang/triton/blob/main/python/tutorials/gluon/08-warp-specialization.py) | Warp-specialized attention: 645L total; FMHA on B200 better than stock Triton but still slower than cuDNN [[8]](#ref-8) |
