...
| **AI Models / Frameworks** | LLaMA 2/3, Mistral, DeepSeek (via FlashAttention-2/3 <sup>[[13]](#ref-13)</sup>); most models via `torch.compile` / vLLM / SGLang; Liger Kernel (LinkedIn) | DeepSeek MLA (FlashMLA <sup>[[14]](#ref-14)</sup>); vLLM integration (2.91x latency vs baseline) | Gemini (Google, TPU via JAX/Flax <sup>[[15]](#ref-15)</sup>); Anthropic, xAI, Apple — training on Google Cloud TPUs via JAX AI Stack | No production models yet (Dec 2025) | No production models yet (beta Oct 2025) | OpenAI production models (internal, undisclosed) <sup>[[8]](#ref-8)</sup> | Modular MAX inference engine; no public LLM integrations yet |
